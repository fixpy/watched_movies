...
movie = Table('movie', post_meta,
    ...
    Column('api_type', String(length=30)),
...
def upgrade(migrate_engine):
    ...
    post_meta.tables['movie'].columns['api_rate'].create()
    post_meta.tables['movie'].columns['api_review'].create()
    post_meta.tables['movie'].columns['api_type'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    post_meta.tables['movie'].columns['api_rate'].drop()
    post_meta.tables['movie'].columns['api_review'].drop()
    post_meta.tables['movie'].columns['api_type'].drop()
